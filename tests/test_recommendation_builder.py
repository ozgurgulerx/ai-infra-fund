...
class RecommendationBuilderTests(unittest.TestCase):
    def test_builds_advisory_artifact_and_audit_from_deterministic_inputs(self) -> None:
        ...
        self.assertEqual(
            ("model-run-summary", "model-run-review"), result.artifact.model_run_ids
        )
        ...
        self.assertEqual(
            "published", result.artifact.final_payload["publication_status"]
        )
        self.assertEqual(
            "deterministic_placeholder_with_audited_model_runs",
            result.artifact.final_payload["narrative_source"],
        )
        ...
        self.assertEqual(
            ...
            result.audit.reviewer_findings,
        )

    def test_suppresses_publication_for_stale_quarantined_or_frozen_inputs(
        self,
    ) -> None:
        result = build_recommendation(
            signal_bundle=signal(),
        ...
        self.assertEqual(
            "suppressed", result.artifact.final_payload["publication_status"]
        )
        self.assertFalse(result.audit.schema_valid)
        self.assertEqual(
            ("evidence-1",), result.audit.deterministic_checks["stale_evidence_ids"]
        )
        self.assertEqual(
            ("evidence-2",),
            result.audit.deterministic_checks["quarantined_evidence_ids"],
        )
    ...
    def test_suppresses_publication_for_missing_relevant_evidence(self) -> None:
        result = build_recommendation(
            ...
            target_weights=target_weights(),
            evidence_claims=(
                claim(evidence_id="evidence-msft", ticker_or_theme="MSFT"),
            ),
            model_run_ids=("model-run-summary",),
            created_at=NOW,
        )
    ...
    def test_suppresses_publication_for_invalid_target_weights_or_validation_failure(
        self,
    ) -> None:
        ...
        unlinked_target = {
            **target_weights_record(),
            "source_signal_bundle_ids": ("other-signal-bundle",),
        }
        ...
        self.assertEqual(
            ("deterministic_validation_failed",), unlinked_result.suppression_reasons
        )
        self.assertFalse(
            unlinked_result.audit.deterministic_checks["source_signal_bundle_linked"]
        )
        ...
        self.assertEqual(
            ("invalid_target_weights",), negative_weight_result.suppression_reasons
        )
        self.assertFalse(
            negative_weight_result.audit.deterministic_checks[
                "target_weights_unit_bounds_valid"
            ]
        )

    def test_event_sink_emits_weights_generated_then_recommendation_issued(
        self,
    ) -> None:
        emitted: list[object] = []
        ...
        self.assertEqual(["weights_generated", "recommendation_issued"], kinds)
        run_ids = [event.run_id for event in emitted]
        self.assertEqual(["run-builder-001", "run-builder-001"], run_ids)
        # ensure builder output is unchanged when a sink is attached
        self.assertTrue(result.should_publish)

    def test_event_sink_is_optional_and_default_behavior_is_unchanged(self) -> None:
        result_with_sink = build_recommendation(
            signal_bundle=signal(),
            target_weights=target_weights(),
            evidence_claims=(claim(),),
            model_run_ids=("model-run-summary",),
        ...
        result_without_sink = build_recommendation(
            signal_bundle=signal(),
            target_weights=target_weights(),
            evidence_claims=(claim(),),
            model_run_ids=("model-run-summary",),
        ...
        self.assertEqual(
            result_with_sink.artifact.recommendation_id,
            result_without_sink.artifact.recommendation_id,
        )

    def test_recommendation_modules_do_not_import_model_clients_or_execution_surface(
        self,
    ) -> None:
        forbidden = (
            "from azure",
