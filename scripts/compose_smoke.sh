...
docker compose exec -T postgres psql \
  ...
  -d ai_infra_fund \
  -v ON_ERROR_STOP=1 \
  -c "SELECT column_name FROM information_schema.columns WHERE table_schema = 'audit' AND table_name = 'backtest_requests' AND column_name IN ('formula_version', 'git_sha', 'model_run_id') ORDER BY column_name;"
docker compose exec -T postgres psql \
  -U ai_infra_fund \
  -d ai_infra_fund \
  -v ON_ERROR_STOP=1 \
...
docker compose exec -T postgres psql \
  ...
  -d ai_infra_fund \
  -v ON_ERROR_STOP=1 \
  -c "SELECT to_regclass('analyst.outcome_journal_entries') AS outcome_journal_entries;"
docker compose exec -T postgres psql \
  -U ai_infra_fund \
  -d ai_infra_fund \
  -v ON_ERROR_STOP=1 \
...
docker compose logs --no-color --tail=100 api worker web migrate

worker_logs="$(docker compose logs --no-color --tail=100 worker || true)"
if printf '%s\n' "$worker_logs" | grep -Eq '(ERROR|Traceback|UndefinedColumn|ImportError)'; then
  log "worker logs contain errors"
  printf '%s\n' "$worker_logs"
  exit 1
fi
