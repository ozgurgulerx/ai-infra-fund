...
const dashboardEndpointConfigs: DashboardEndpointConfig[] = [
  {
    ...
    plannedDetail:
      "Dashboard repository is configured, but no DB-backed records are available yet.",
    buildVisibleValue: (payload) =>
      String(payload.status ?? "available").toUpperCase(),
    buildDetail: (payload) => {
    ...
    statusOverride: (payload) =>
      payload.status === "empty" ? "planned" : undefined,
  },
  {
    key: "evidence",
    ...
    title: "Evidence",
    plannedDetail:
      "Evidence tables are reachable, but no ingested records have been reported yet.",
    buildVisibleValue: (payload) =>
      `${readNumber(payload, "total_items")} items`,
    buildDetail: (payload) =>
  ...
  },
  {
    ...
    plannedDetail:
      "Recommendation artifacts are intentionally empty until audited advisory outputs exist.",
    buildVisibleValue: (payload) =>
      `${readNumber(payload, "total_recommendations")} artifacts`,
    buildDetail: (payload) =>
  ...
  },
  {
    ...
    title: "Evaluation",
    plannedDetail:
      "Backtest and run-artifact tables are ready; no evaluation output has been published.",
    buildVisibleValue: (payload) =>
      `${readNumber(payload, "total_backtest_runs", "total_backtests")} backtests`,
    buildDetail: (payload) =>
  ...
  },
  {
    ...
    title: "Model Runs",
    plannedDetail:
      "ModelRun ledger is available, with no recorded runs in the current feed.",
    buildVisibleValue: (payload) =>
      `${readNumber(payload, "total_model_runs", "total_runs")} runs`,
    buildDetail: (payload) =>
      `Runs by status: ${formatBreakdown(payload.runs_by_status)}.`,
  },
  {
    key: "dataQuality",
    ...
    title: "Data Quality",
    plannedDetail:
      "Data-quality checks are intentionally empty until scheduled checks publish results.",
    buildVisibleValue: (payload) =>
      `${readNumber(payload, "total_checks")} checks`,
    buildDetail: (payload) =>
  ...
  },
  {
    ...
    title: "Incidents",
    plannedDetail:
      "Incident records are empty; no active freeze is exposed by the feed.",
    buildVisibleValue: (payload) =>
  ...
  },
  {
    ...
    plannedDetail:
      "Watchlist universe is reachable, but no tickers are registered yet.",
    buildVisibleValue: (payload) =>
      `${readNumber(payload, "total_members")} tickers`,
    buildDetail: (payload) =>
      `Status mix: ${formatBreakdown(payload.by_watchlist_status)}.`,
  },
  {
    key: "crawlFreshness",
    ...
    buildVisibleValue: (payload) =>
      `${readNumber(payload, "total_datasets")} datasets`,
    buildDetail: (payload) =>
      `Latest available ${readOptionalText(payload, "latest_available_at")}.`,
  },
  {
    key: "equityEvents",
    ...
    buildVisibleValue: (payload) =>
      `${readArray(payload, "events").length} events`,
  ...
  },
  {
    ...
    buildVisibleValue: (payload) =>
      `${readArray(payload, "snapshots").length} score cards`,
...
export default function Page() {
  ...
  const [dashboardFeeds, setDashboardFeeds] = useState<DashboardFeedMap>(
    initialDashboardFeeds,
  );
  const [moduleSummaries, setModuleSummaries] = useState<
    DashboardModuleSummary[]
  >([]);
  const [advisoryChain, setAdvisoryChain] =
    useState<AdvisoryChainPayload>(initialAdvisoryChain);
  ...
  return (
    <AppShell
      eyebrow="Local Read-Only Dashboard"
      title="AI Infrastructure Fund Control Room"
    >
      <div className="control-room-shell">
        <section className="status-strip" aria-label="System Status">
          ...
          <StatusTile
            title="Readiness"
            status={
              readiness.state === "available" && readiness.status === "ready"
                ? "passing"
                : "degraded"
            }
        ...
        <div className="dashboard-grid">
          ...
          >
            ...
            <p className="panel-note">
              Portfolio rows remain a static planning snapshot until a read-only
              portfolio summary feed is exposed. Manual trade entry and Trade
              Journal views stay disabled in this read-only phase.
            </p>
          </SectionPanel>
          ...
          >
            <div className="audit-stack">
              <div>
                ...
                <span>
                  Evidence IDs, Model Runs, SignalBundle, TargetWeights, and
              ...
              <div>
                ...
                <span>
                  No recommendation generation from the browser; artifacts
                  remain read-only.
                </span>
              </div>
              <div>
                ...
                <span>
                  No transaction controls, venue links, credentials, or
                  submission surface.
                </span>
              </div>
            </div>
          ...
          >
            <div className="compact-list">
              <p className="chain-path">{advisoryChainPath}</p>
              <p>
                <strong>{chainHeadline(advisoryChain)}</strong>{" "}
                {chainDetail(advisoryChain)}
              </p>
            </div>
            <div className="status-list chain-list">
              <ChainLinkRow
                label="evidence_id"
                value={chainId(advisoryChain, "evidence_id")}
              />
              <ChainLinkRow
                label="chunk_id"
                value={chainId(advisoryChain, "chunk_id")}
              />
              <ChainLinkRow
                label="claim_id"
                value={chainId(advisoryChain, "claim_id")}
              />
              <ChainLinkRow
                label="model_run_ids"
                value={chainId(advisoryChain, "model_run_ids")}
              />
              <ChainLinkRow
                label="signal_bundle_id"
                value={chainId(advisoryChain, "signal_bundle_id")}
              />
              <ChainLinkRow
                label="target_weights_id"
                value={chainId(advisoryChain, "target_weights_id")}
              />
              <ChainLinkRow
                label="recommendation_id"
                value={chainId(advisoryChain, "recommendation_id")}
              />
              <ChainLinkRow
                label="audit_id"
                value={chainId(advisoryChain, "audit_id")}
              />
              <ChainLinkRow
                label="evaluation"
                value={chainEvaluation(advisoryChain)}
              />
            </div>
          </SectionPanel>
          ...
          <SectionPanel eyebrow="Universe" title="Watchlist Status">
            <div className="compact-list">
              <p>
                <strong>{dashboardFeeds.watchlist.visibleValue}</strong>{" "}
                {dashboardFeeds.watchlist.detail}
              </p>
          ...
          <SectionPanel eyebrow="Freshness" title="Crawl Freshness">
            <div className="compact-list">
              <p>
                <strong>{dashboardFeeds.crawlFreshness.visibleValue}</strong>{" "}
                {dashboardFeeds.crawlFreshness.detail}
              </p>
          ...
          <SectionPanel eyebrow="Events" title="Latest Equity Events">
            <div className="compact-list">
              <p>
                <strong>{dashboardFeeds.equityEvents.visibleValue}</strong>{" "}
                {dashboardFeeds.equityEvents.detail}
              </p>
          ...
          >
            <div className="status-list">
              {scoreCards(dashboardFeeds.signalSnapshots.payload).map(
                (card) => (
          ...
          >
            <div className="compact-list">
              <p>
                <strong>{dashboardFeeds.advisoryRun.visibleValue}</strong>{" "}
                {dashboardFeeds.advisoryRun.detail}
              </p>
          ...
          <SectionPanel eyebrow="Ticker" title="Ticker Intelligence">
            <div className="compact-list">
              <p>
                <strong>
                  {dashboardFeeds.tickerIntelligence.visibleValue}
                </strong>{" "}
                {dashboardFeeds.tickerIntelligence.detail}
              </p>
          ...
          <SectionPanel eyebrow="Traceability" title="Evidence And Audit Trace">
            <div className="status-list">
              <TraceRow
                label="evidence_ids"
                value={traceValue(
                  dashboardFeeds.tickerIntelligence.payload,
                  "evidence_ids",
                )}
              />
              <TraceRow
          ...
          <SectionPanel eyebrow="Evidence" title="Evidence Ingestion Status">
            <div className="compact-list">
              <p>
                <strong>{dashboardFeeds.evidence.visibleValue}</strong>{" "}
                {dashboardFeeds.evidence.detail}
              </p>
          ...
          <SectionPanel eyebrow="Evaluation" title="Backtest And Evaluation">
            <div className="compact-list">
              <p>
                <strong>{dashboardFeeds.evaluation.visibleValue}</strong>{" "}
                {dashboardFeeds.evaluation.detail}
              </p>
          ...
          <SectionPanel
            eyebrow="Operations"
            title="Model Runs, Data Quality, Incidents"
          >
            <div className="status-list">
...
function DashboardFeedRow({
  feed,
}: {
  feed: DashboardSummaryFeed<DashboardPayload>;
}) {
  return (
...
async function fetchDashboardFeedMap(): Promise<DashboardFeedMap> {
  const entries = await Promise.all(
    dashboardEndpointConfigs.map(
      async (config) => [config.key, await fetchDashboardFeed(config)] as const,
    ),
  );
...
async function fetchDashboardFeed(
...
): Promise<DashboardSummaryFeed<DashboardPayload>> {
  ...
  try {
    ...
    if (!response.ok) {
      return createDegradedDashboardFeed(
        config,
...
function normalizeDashboardFeed(
  ...
  const status =
    config.statusOverride?.(payload) ??
    moduleStatusFromSummary(readOptionalText(payload, "status"));
...
function createPlannedDashboardFeed(
  config: DashboardEndpointConfig,
): DashboardSummaryFeed<DashboardPayload> {
  return {
    state: "available",
...
function eventHeadline(events: Record<string, unknown>[]): string {
  ...
  return first
    ? `${textValue(first, "title", "Untitled event")} / ${textValue(first, "evidence_id", "no evidence_id")}.`
    : "No events reported.";
}
...
function tickerTraceHeadline(payload: DashboardPayload): string {
  const recommendation = isRecord(payload.latest_recommendation)
  ...
  return [
    {
      ...
      value: textValue(snapshot, "sentiment_score", "not reported"),
    },
    {
      label: "technical_score",
      value: textValue(snapshot, "technical_score", "not reported"),
    },
    {
      label: "fundamental_score",
      value: textValue(snapshot, "fundamental_score", "not reported"),
    },
  ];
}
...
function traceValue(
  ...
  const recommendation =
    payload && isRecord(payload.latest_recommendation)
      ? payload.latest_recommendation
      : undefined;
...
function feedHasNoRecords(payload: DashboardPayload): boolean {
  const totalEntries = Object.entries(payload).filter(
    ([key, value]) => key.startsWith("total_") && typeof value === "number",
  );
  return (
    totalEntries.length > 0 && totalEntries.every(([, value]) => value === 0)
  );
}
